...
async function sendWelcomeEmail(email: string, strapi: any): Promise<void> {
  ...
  try {
    ...
    if (error) {
      strapi.log.error('Resend API error:', error);
      throw error;
    }

    strapi.log.info(`Welcome email sent successfully to ${email}`, data);
  } catch (error) {
      ...
      if (existing) {
        if (existing.status === 'active') {
          ...
          try {
            ...
            strapi.log.error('Failed to send welcome email on reactivation:', emailError);
            // 不影响重新激活成功
          }
